...
if (! function_exists('happyforms_get_form_partial')):
	...
	function happyforms_get_form_partial($partial_name, $form) {
		$safe_partial = sanitize_file_name(wp_basename($partial_name));
		$include_dir = happyforms_get_include_folder() . '/templates/partials';
		$core_dir = happyforms_get_core_folder() . '/templates/partials';

		if (empty($safe_partial)) {
			return '';
		}

		$candidate = $include_dir . '/' . $safe_partial . '.php';

		if (! file_exists($candidate)) {
			$candidate = $core_dir . '/' . $safe_partial . '.php';
		}
